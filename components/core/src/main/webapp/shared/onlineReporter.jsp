<jsp:root version="2.0"
  ...
  <mm:content postprocessor="none"
    ...
    <mm:cloud method="asis">
      <mm:import id="givenuser" externid="user" from="parameters" />
      <mm:cloudinfo type="usernode" id="cloudUser" write="false" />

      <c:choose>
        <c:when test="${(givenuser eq 0 and empty cloudUser) or (cloudUser eq givenuser)}">
          <online>
        ...
        <c:otherwise>
          <offline>
            <mm:log level="info">Logging out ${cloudUser} != ${givenuser}</mm:log>
            <jsp:text>${cloudUser} != ${givenuser}</jsp:text>
          </offline>
        </c:otherwise>
      </c:choose>
